...
export class Tournament {
    ...
    awardPrize(character, result, prize, isFinalTable = false) {
        ...
        // 결과 기록 (NPC 결과도 함께 기록)
        this.results = [...(result.npcResults || []), finalResult];
    ...
    enter(character) {
        ...
        // 파이널 테이블 진출한 경우, 상금 지급 없이 결과만 반환
        return {
            success: true,
            finalTable: true,
            result: {
                ...result,
                prize: 0, // 상금은 아직 지급하지 않음
                isInMoney: this.prizeSystem.isInMoney(result.rank, this.type),
                isFinalTable: true,
                score: result.score,
                prizePool: this.prizePool,
    ...
    calculateInitialResult(character) {
        // NPC 결과 생성
        const npcResults = this.npcSystem.generateNPCResults(
            this.maxPlayers - 1
        ); // 플레이어 1명을 고려하여 -1
        ...
        const allScores = [
            ...npcResults.map((r) => ({ score: r.score, isPlayer: false })),

...
export class CharacterUI {
    ...
    createProfileCard() {
        ...
        stats.innerHTML = `
            ${this.language.t(
                "stats.bankroll"
            )}: $${this.character.stats.bankroll.toLocaleString()}<br>
            ${this.language.t(
                "stats.reputation"
            )}: ${this.character.stats.reputation.toLocaleString()}
        `;
    ...
    mount(parentElement) {
        ...
        // 언어 변경 시 UI 업데이트
        this.language.addObserver(() => {
            const newContainer = this.createProfileCard();
            this.container.parentElement.replaceChild(
                newContainer,
                this.container
            );

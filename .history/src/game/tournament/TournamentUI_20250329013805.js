...
export class TournamentUI {
    ...
    updateUI() {
        ...
        // 언어 버튼 생성 함수
        const createLangButton = (lang, text) => {
            const button = document.createElement("button");
            const isSelected = this.language.getCurrentLang() === lang;
            button.style.cssText = `
                padding: 5px 10px;
                background: ${isSelected ? "rgb(76, 175, 80)" : "transparent"};
                border: 1px solid ${isSelected ? "rgb(76, 175, 80)" : "#666"};
                border-radius: 4px;
        ...
        // 언어 버튼 추가
        langContainer.appendChild(createLangButton("en", "English"));
        langContainer.appendChild(createLangButton("ko", "한국어"));
        langContainer.appendChild(createLangButton("ja", "日本語"));
        langContainer.appendChild(createLangButton("zh", "中文"));

export class TournamentNPC {
    ...
    generateNPCSkill() {
        ...
        switch (this.type) {
            case "online":
                // 온라인 스페셜은 매우 낮은 난이도 (100-250)
                baseSkill = 100 + Math.random() * 150;
                break;
            case "highroller":
                // 하이롤러는 중간 난이도 (200-350)
                baseSkill = 200 + Math.random() * 150;
                break;
            default:
                baseSkill = 100 + Math.random() * 150;
        }
    ...
    generateNPCResults(count) {
        ...
        for (let i = 0; i < npcs.length; i++) {
            ...
            results.push({
                isNPC: true,
                rank: i + 1,
                score: (npc.skill / 1000) * 0.3 + npc.luckFactor * 0.7, // 스킬 30% + 운 70%
            });
        }

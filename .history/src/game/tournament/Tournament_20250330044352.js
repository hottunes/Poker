...
export class Tournament {
    ...
    enter(character) {
        ...
        // 파이널 테이블 진출하지 않은 경우에만 여기서 상금 지급 (10위 이하)
        if (result.rank > 9 && result.prize > 0) {
            character.resources.bankroll = Math.floor(
                character.resources.bankroll + result.prize
            );
    ...
    getFinalTableResult(character, initialResult) {
        // 상금 계산 (파이널 테이블에서는 다시 계산하지 않고 초기 결과의 상금을 사용)
        const prize = initialResult.prize;
        ...
        // 상금 지급 (파이널 테이블 진출자의 경우에만 여기서 지급, 9등 이내)
        if (prize > 0 && initialResult.rank <= 9) {
            character.resources.bankroll = Math.floor(
                character.resources.bankroll + prize
            );
        }

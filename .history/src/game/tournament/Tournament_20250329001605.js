export class Tournament {
    ...
    calculateScore(character) {
        ...
        // 기본 스킬 점수 계산 (각 스탯을 먼저 정규화)
        let skillScore = 0;
        skillScore += (character.stats.strategy / 1000) * weights.strategy * 80;
        skillScore += (character.stats.analysis / 1000) * weights.analysis * 80;
        skillScore += (character.stats.insight / 1000) * weights.insight * 80;
        skillScore +=
            (character.stats.mentalFocus / 1000) * weights.mentalFocus * 80;
        skillScore += (character.stats.stamina / 1000) * weights.stamina * 80;

        // 토너먼트 진행 상황에 따라 분석력의 중요도 증가
        const remainingPlayersRatio = this.currentPlayers / this.maxPlayers;
        const analysisBonus =
            (1 - remainingPlayersRatio) * (character.stats.analysis / 1000) * 5;
        skillScore += analysisBonus;

        // 운 점수 계산 (0-20점 범위로 축소)
        // Box-Muller 변환을 사용한 정규분포 생성 (평균 10, 표준편차 3)
        const u1 = Math.random();
        const u2 = Math.random();
        const z = Math.sqrt(-2.0 * Math.log(u1)) * Math.cos(2.0 * Math.PI * u2);
        const luckScore = Math.max(0, Math.min(20, 10 + z * 3));

        // 최종 점수 = 스킬 점수(80%) + 운 점수(20%)
        const finalScore = skillScore + luckScore;
    ...
    generateNPCScore() {
        // NPC 스킬 레벨 (0-80점 범위)
        const baseSkillLevel = 40; // 기본 스킬 레벨 (500 스탯에 해당, 50% 수준)

        // 스킬 레벨에 약간의 변동성 추가 (±8점)
        const skillVariation = Math.random() * 16 - 8;
        const skillScore = Math.max(
            0,
            Math.min(80, baseSkillLevel + skillVariation)
        );

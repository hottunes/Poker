export class Tournament {
    ...
    calculateInitialResult(character) {
        ...
        const prize = 0; // 파이널 테이블 진출 전에는 상금 미정
    ...
    calculateITMBonus(rank, totalPlayers) {
        const itmCount = Math.floor(totalPlayers * 0.1); // ITM 비율을 10%로 수정 (100위)
    ...
    getLogStats(playerName) {
        ...
        const stats = {
            ...
            itmCount: 0,
            winCount: 0,
            finalTableCount: 0, // 파이널 테이블 진출 횟수 추가
        };
        ...
        playerLogs.forEach((log) => {
            ...
            if (log.rank <= Math.floor(log.totalPlayers * 0.1))
                // ITM 비율을 10%로 수정
                stats.itmCount++;
            if (log.rank === 1) stats.winCount++;
            if (log.rank <= 9) stats.finalTableCount++; // 파이널 테이블 진출 체크
        });

export class Tournament {
    ...
    calculateFinalTableScore(character) {
        ...
        const skillBonus =
            (stats.strategy * 0.35 + // 전략이 가장 중요
                stats.insight * 0.25 + // 통찰력
                stats.mentalFocus * 0.15 + // 정신적 집중력
                stats.stamina * 0.05 + // 체력
                stats.analysis * 0.2) / // 분석력 - 파이널 테이블에서 매우 중요
            999; // 999로 나누어 정규화

        const baseScore = 50 + (Math.random() * 30 - 15); // 35-65 범위
        const skillScore = skillBonus * 20; // 스킬의 영향력 증가
    ...
    calculateScore(character) {
        // 각 스탯의 가중치 설정
        const weights = {
            pokerIQ: 0.35, // 포커 IQ (가장 중요)
            insight: 0.25, // 통찰력
            gtoMastery: 0.2, // GTO 마스터리
            focus: 0.15, // 집중력
            stamina: 0.05, // 체력
            analysis: 0.2, // 분석력 - 토너먼트 후반부에서 더 중요해짐
        };

        // 기본 점수 계산
        let score = 0;
        score += character.stats.strategy * weights.pokerIQ;
        score += character.stats.insight * weights.insight;
        score += character.stats.mentalFocus * weights.focus;
        score += character.stats.stamina * weights.stamina;
        score += character.stats.analysis * weights.analysis;

        // 토너먼트 진행 상황에 따라 분석력의 중요도 증가
        // 참가자가 적을수록(후반부일수록) 분석력이 더 중요해짐
        const remainingPlayersRatio = this.currentPlayers / this.maxPlayers;
        const analysisBonus =
            (1 - remainingPlayersRatio) * character.stats.analysis * 0.2;
        score += analysisBonus;
    ...
    generateNPCScore() {
        ...
        // 고정된 NPC 스킬 레벨 (0-1 사이 값)
        const npcSkillLevel = 0.6;

        // 스킬 보너스 (최대 20점)
        const skillScore = npcSkillLevel * 20;

        // 운 요소를 정규분포로 변경 (평균 0, 표준편차 15)
        const u3 = Math.random();
        const u4 = Math.random();
        const z2 =
            Math.sqrt(-2.0 * Math.log(u3)) * Math.cos(2.0 * Math.PI * u4);
        const luckFactor = z2 * 15;

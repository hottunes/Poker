...
export class TournamentUI {
    ...
    // 토너먼트 참가 처리
    async enterTournament(type, buyIn) {
        // 이미 처리 중이면 리턴
        ...
        const tournament = new Tournament(type, buyIn, 100);
        ...
        // 결과창 생성
        this.createResultContainer(result, tournament);
    ...
    // 결과창 생성
    createResultContainer(result, tournament) {
        // 기존 결과창 제거
        this.removeResultContainer();

        this.resultContainer = document.createElement("div");
        this.resultContainer.className = "tournament-result";

        // 순위 표시
        const rankText = document.createElement("h2");
        rankText.textContent = `${result.result.rank}등`;
        this.resultContainer.appendChild(rankText);
        ...
        if (result.result.prize > 0) {
            ...
            prizeText.textContent = `상금: ${result.result.prize.toLocaleString()}`;
            statsContainer.appendChild(prizeText);
        }

        this.resultContainer.appendChild(statsContainer);

        // 파이널 테이블 진출 시 결과 버튼 표시
        if (result.finalTable) {
            const resultButton = document.createElement("button");
            resultButton.textContent = "결과 확인";
            resultButton.onclick = () =>
                this.showFinalTableResult(tournament, result.result);
            this.resultContainer.appendChild(resultButton);
        }

        // 닫기 버튼
        const closeButton = document.createElement("button");
        closeButton.textContent = "닫기";
        closeButton.onclick = () => this.removeResultContainer();
    ...
    // 파이널 테이블 결과 표시
    showFinalTableResult(tournament, initialResult) {
        const finalResult = tournament.getFinalTableResult(
            this.game.character,
            initialResult
        );
        ...
        // 새로운 결과창 생성
        this.resultContainer = document.createElement("div");
        this.resultContainer.className = "tournament-result final-table";

        // 파이널 테이블 결과 표시
        const rankText = document.createElement("h2");
        rankText.textContent = `파이널 테이블 ${finalResult.result.rank}등`;
        this.resultContainer.appendChild(rankText);
        ...
        if (finalResult.result.prize > 0) {
            ...
            this.resultContainer.appendChild(prizeText);
        }

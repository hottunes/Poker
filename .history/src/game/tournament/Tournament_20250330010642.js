...
export class Tournament {
    ...
    generateNPCPlayers(count) {
        ...
        for (let i = 0; i < count; i++) {
            // NPC 기본 능력치를 100-200 사이로 더 낮춤
            const baseSkill = Math.floor(Math.random() * 100) + 100;

            // 변동폭을 더 크게 설정 (-100 ~ +150)
            const variance = Math.floor(Math.random() * 250) - 100;

            // 최종 능력치 계산 (최소 50, 최대 400)

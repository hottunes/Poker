export class Tournament {
    ...
    calculateScore(character) {
        ...
        // 스킬 점수를 0-70점 범위로 정규화 (1000을 기준으로)
        skillScore = (skillScore / 1000) * 70;

        // 운 점수 계산 (0-30점 범위)
        // Box-Muller 변환을 사용한 정규분포 생성 (평균 15, 표준편차 5)
        const u1 = Math.random();
        const u2 = Math.random();
        const z = Math.sqrt(-2.0 * Math.log(u1)) * Math.cos(2.0 * Math.PI * u2);
        let luckScore = Math.max(0, Math.min(30, 15 + z * 5));

        // 실력에 따른 운 관리 시스템
        const skillLevel = skillScore / 70; // 0-1 사이의 실력 레벨

        if (luckScore < 15) {
            // 운이 나쁠 때
            // 실력이 높을수록 운이 나쁜 상황에서 손실 최소화
            luckScore =
                luckScore * (1 - skillLevel * 0.3) + 15 * (skillLevel * 0.3);
        } else {
            // 운이 좋을 때
            // 실력이 높을수록 운이 좋은 상황에서 이득 최대화
            luckScore = luckScore * (1 + skillLevel * 0.2);
        }

        // 최종 점수 = 스킬 점수(70%) + 관리된 운 점수(30%)
        const finalScore = skillScore + luckScore;
    ...
    generateNPCScore() {
        // NPC 스킬 레벨 (0-70점 범위)
        const baseSkillLevel = 35; // 기본 스킬 레벨 (500 스탯에 해당, 50% 수준)

        // 스킬 레벨에 약간의 변동성 추가 (±7점)
        const skillVariation = Math.random() * 14 - 7;
        const skillScore = Math.max(
            0,
            Math.min(70, baseSkillLevel + skillVariation)
        );
        ...
        // NPC의 실력에 따른 운 관리
        const skillLevel = skillScore / 70;

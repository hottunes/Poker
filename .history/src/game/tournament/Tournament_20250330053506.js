...
export class Tournament {
    ...
    // 상금 지급 및 결과 기록
    awardPrize(character, result, isFinalTable = false) {
        // 상금 계산
    ...
    enter(character) {
        ...
        if (!result.isFinalTable) {
            const finalResult = this.awardPrize(character, result, false);
            return {
                success: true,
                finalTable: false,
                result: finalResult,
    ...
    getFinalTableResult(character, initialResult) {
        ...
        // 상금 지급 및 결과 기록
        const finalResult = this.awardPrize(character, initialResult, true);

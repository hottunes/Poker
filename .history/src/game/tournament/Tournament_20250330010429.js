...
export class Tournament {
    ...
    calculateSkill(character) {
        ...
        // 각 스탯을 0~1 범위로 정규화 (500을 0.6으로 매핑)
        const normalized = {
            strategy: (character.stats.strategy / 833) * 0.6,
            analysis: (character.stats.analysis / 833) * 0.6,
            insight: (character.stats.insight / 833) * 0.6,
            mentalFocus: (character.stats.mentalFocus / 833) * 0.6,
            stamina: (character.stats.stamina / 833) * 0.6,
        };
    ...
    generateNPCScore() {
        ...
        switch (this.type) {
            case "online":
                baseSkill = 0.3; // 온라인: 낮은 실력
                break;
            case "major":
                baseSkill = 0.5; // 메이저: 평균 실력
                break;
            default:
                baseSkill = 0.35; // 일반: 낮은-평균 실력
        }

        // 더 큰 변동성 추가 (±40%)
        const skillVariation = Math.random() * 0.8 - 0.4;
    ...
    generateNPCPlayers(count) {
        ...
        for (let i = 0; i < count; i++) {
            // NPC 기본 능력치를 200-300 사이로 더 낮춤
            const baseSkill = Math.floor(Math.random() * 100) + 200;

            // 변동폭을 더 크게 설정 (-100 ~ +100)
            const variance = Math.floor(Math.random() * 200) - 100;

            // 최종 능력치 계산 (최소 100, 최대 500)
            const finalSkill = Math.max(
                100,
                Math.min(500, baseSkill + variance)
            );

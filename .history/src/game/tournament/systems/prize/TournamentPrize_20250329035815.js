export class TournamentPrize {
    constructor(tournament) {
        this.tournament = tournament;
        this.prizeStructure = {
            1: 0.23,
            2: 0.135,
            3: 0.085,
            4: 0.065,
            5: 0.055,
            6: 0.039,
            7: 0.029,
            8: 0.019,
            9: 0.013,
            10: 0.01,
            "11-15": 0.01,
            "16-20": 0.007,
            "21-25": 0.006,
            "26-30": 0.005,
            "31-35": 0.0045,
            "36-40": 0.004,
            "41-50": 0.0028,
            "51-60": 0.0024,
            "61-75": 0.0022,
            "76-100": 0.0021,
        };
    }

    // 순위에 따른 상금 비율 계산
    getPrizeShare(rank) {
        if (this.prizeStructure[rank]) {
            return this.prizeStructure[rank];
        }
        // 범위 형태의 key 확인
        for (let key in this.prizeStructure) {
            if (key.includes("-")) {
                let [low, high] = key.split("-").map(Number);
                if (rank >= low && rank <= high) {
                    return this.prizeStructure[key];
                }
            }
        }
        return 0; // 상금 구조에 포함되지 않으면 0 지급
    }

    // 순위별 상금 계산
    calculatePrize(rank) {
        const totalPrize = this.tournament.buyIn * this.tournament.maxPlayers;

        if (this.tournament.maxPlayers === 1000) {
            return this.calculatePrize1000(rank, totalPrize);
        } else {
            return this.calculatePrize200(rank, totalPrize);
        }
    }

    calculatePrize1000(rank, totalPrize) {
        const prizePercentages = {
            1: 0.3, // 30%
            2: 0.2, // 20%
            3: 0.15, // 15%
            4: 0.1, // 10%
            5: 0.07, // 7%
            6: 0.06, // 6%
            7: 0.05, // 5%
            8: 0.04, // 4%
            9: 0.03, // 3%
        };

        return Math.floor(totalPrize * (prizePercentages[rank] || 0));
    }

    calculatePrize200(rank, totalPrize) {
        const prizePercentages = {
            1: 0.4, // 40%
            2: 0.25, // 25%
            3: 0.15, // 15%
            4: 0.1, // 10%
            5: 0.05, // 5%
            6: 0.03, // 3%
            7: 0.02, // 2%
        };

        return Math.floor(totalPrize * (prizePercentages[rank] || 0));
    }

    // ITM 보너스 계산
    calculateITMBonus(rank, totalPlayers) {
        const baseBonus = 100;

        if (totalPlayers === 1000) {
            // 1000명 토너먼트의 경우 100위까지 ITM
            if (rank > 100) return 0;
            return Math.floor((baseBonus * (100 - rank + 1)) / 100);
        } else {
            // 100명 토너먼트의 경우 기본 상금 구조 사용
            const prizeStructure = [0.25, 0.15, 0.1, 0.08, 0.06];
            if (rank > prizeStructure.length) return 0;
            return Math.floor(
                (baseBonus * (prizeStructure.length - rank + 1)) /
                    prizeStructure.length
            );
        }
    }
}

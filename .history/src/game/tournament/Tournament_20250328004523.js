export class Tournament {
    ...
    calculateScore(character) {
        ...
        // 정규분포를 사용한 기본 점수 생성 (평균 50, 표준편차 15)
        let baseScore = 0;
        for (let i = 0; i < 6; i++) {
            // 중심극한정리를 이용한 정규분포 근사
            baseScore += Math.random() * 100;
        }
        baseScore = baseScore / 6; // 평균을 50으로 조정

        // 스킬 보너스 (최대 30점)
        const skillScore = skillBonus * 30;

        // 운 요소 (-15 ~ +15)
        const luckFactor = Math.random() * 30 - 15;
    ...
    generateNPCScore() {
        ...
        // NPC 스킬 레벨 (0-1 사이)
        const npcSkillLevel = Math.random(); // 균등 분포의 스킬 레벨
        const skillScore = npcSkillLevel * 30; // 최대 30점의 스킬 보너스

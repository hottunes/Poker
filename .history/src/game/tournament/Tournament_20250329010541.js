export class Tournament {
    ...
    enter(character) {
        ...
        if (initialResult.rank <= 9) {
            return {
                ...
                pendingResult: initialResult, // 결과를 바로 보여주지 않고 저장
            };
        }
    ...
    // 파이널 테이블 결과 확인
    confirmFinalTableResult(pendingResult) {
        // 결과 기록
        this.results.push(pendingResult);
        this.addLog(pendingResult.character, pendingResult);

        // 상금과 명성 지급
        if (pendingResult.prize > 0) {
            pendingResult.character.modifyResource(
                "bankroll",
                pendingResult.prize
            );
            const reputationGain =
                Math.floor(pendingResult.prize / 100) +
                this.calculateITMBonus(pendingResult.rank, this.maxPlayers);
            pendingResult.character.modifyResource(
                "reputation",
                reputationGain
            );
    ...
    calculateInitialResult(character) {
        ...
        const prize = Math.floor(this.totalPrize * prizeShare);
    ...
    calculatePrize(rank) {
        if (this.maxPlayers !== 1000) {
            ...
            if (rank <= prizeStructure.length) {
                return Math.floor(this.totalPrize * prizeStructure[rank - 1]);
            }
            return 0;
        }

        // 1000명 토너먼트의 경우 constructor에서 정의된 prizeStructure 사용
        return Math.floor(this.totalPrize * this.getPrizeShare(rank));
    }
